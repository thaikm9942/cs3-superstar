...
void gravity_hazard_init(Vector position, double mass, Body* player, Scene* scene){
    ...
    grav_body = gravity_ball_init(position, HAZARD_RADIUS, HAZARD_MASS, GRAV_COLOR, 1);
...
void moving_ball_hazard_init(Vector position, Vector velocity, double mass, Scene* scene){
    ...
    moving_ball_body = moving_ball_init(position, HAZARD_RADIUS, mass, BALL_COLOR, 1);
...
void spring_platforms_hazard_init(Vector position1, Vector position2, Vector dimension, double k, Scene* scene){
    ...
    spring_platform_body1 = platform_spring_init(position1, dimension);
    spring_platform_body2 = platform_spring_init(position2, dimension);
